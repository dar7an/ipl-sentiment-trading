"""VADER + cricket-lexicon baseline — offline fallback and A/B comparison."""
...
_QUOTE_BANNED = (
    "fuck", "dick", "shit", "cunt", "nigger", "asshole", "porn",
    "suck my", "bitch", "wtf", "stfu",
)
    ...
    def aggregate_interval(
        ...
        def summarize(rows: list[tuple[float, int, str]]) -> SentimentBucket:
            ...
            return SentimentBucket(
                mean=mean,
                volume=len(rows),
                effective_volume=float(len(rows)),
